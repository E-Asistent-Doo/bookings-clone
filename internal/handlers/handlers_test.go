...
func TestRepository_PostReservation(t *testing.T) {
	...
	//Becouse of validation requires min length of 3
	reqBody = fmt.Sprintf("%s&%s", reqBody, "first_name=Jo")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "last_name=Smith")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "phone=555 444 333 222")

	req, _ := http.NewRequest("POST", "/make-reservation", strings.NewReader(reqBody))

	// get the context, and put the reservation in the session
	ctx := getCtx(req)
	req = req.WithContext(ctx)
	session.Put(ctx, "reservation", reservation)
	...
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	session.Remove(req.Context(), "reservation")
	...
	reqBody = fmt.Sprintf("%s&%s", reqBody, "end_date=2050-01-02")
	reqBody = fmt.Sprintf("%s&%s", reqBody, "email=john@smith.com")
	//This part is under validation part so it wouldn't pass if length is less then 3
	...
	//This is real session check
	reservationInSession := session.Pop(ctx, "reservation").(models.Reservation)

	if reservationInSession.RoomID != mockReservation().RoomID {
		t.Error("Reservation not stored in session")
	}

	//This is my own session simulation whhich is unnecesery
	expectedReservation := mockReservation()

	var mockedSession *mockSession = newMockSession()

	mockedSession.Put("reservation", &reservation)

	if !mockedSession.IsPutCalledWith("reservation", expectedReservation) {
		t.Error("Reservation not stored in session")
	}

	if rr.Result().StatusCode != http.StatusSeeOther {
		t.Errorf("Expected status code %d, got %d", http.StatusSeeOther, rr.Result().StatusCode)
	}

	expectedLocation := "/reservation-summary"

	if got := rr.Result().Header.Get("Location"); got != expectedLocation {
		t.Errorf("Expected redirect location %s, got %s", expectedLocation, got)
	}
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	*****************************************/
	...
	postedData = url.Values{}
	postedData.Add("start_date", "2060-01-01")
	postedData.Add("end_date", "2060-01-02")
	req, _ = http.NewRequest("POST", "/search-availability", strings.NewReader(postedData.Encode()))
...
type mockSession struct {
	putCalledWith map[string]interface{}
}

func (s *mockSession) Put(key string, value interface{}) {
	s.putCalledWith[key] = value
}

func (s *mockSession) IsPutCalledWith(key string, value interface{}) bool {

	v, ok := s.putCalledWith[key]
	if !ok {
		return false
	}
	// Assert the type of v to be *models.Reservation
	reservation, ok := v.(*models.Reservation)
	if !ok {
		return false // Return false if the type assertion fails
	}

	// Compare the RoomID field of the reservations
	expectedReservation := value.(*models.Reservation)
	return reservation.RoomID == expectedReservation.RoomID
}

func newMockSession() *mockSession {
	return &mockSession{
		putCalledWith: make(map[string]interface{}),
	}
}

func mockReservation() *models.Reservation {
...
func TestLogin(t *testing.T) {
	...
	for _, e := range loginTests {
		...
		postedData.Add("password", "coacarf1")
...
func TestAdminPostShowReservation(t *testing.T) {
	for _, e := range adminPostShowReservationTests {
		...
		// call the handler
		handler := http.HandlerFunc(Repo.AdminPostShowsReservation)
		handler.ServeHTTP(rr, req)
