...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end string, roomID int) (bool, error) {
...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	_, err := m.DB.ExecContext(ctx, query, u.FirstName, u.LastName, u.Email, u.AccessLevel, u.UpdatedAt)
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	if err == bcrypt.ErrMismatchedHashAndPassword {
		return 0, "", errors.New("missmatch password")
	} else if err != nil {
		return 0, "", err
	}
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&i.ID, &i.FirstName, &i.LastName, &i.Email, &i.Phone,
			&i.StartDate, &i.EndDate, &i.RoomID, &i.CreatedAt, &i.UpdatedAt, &i.Processed, &i.Room.ID, &i.Room.RoomName)
...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	err := row.Scan(&reservation.ID, &reservation.FirstName, &reservation.LastName, &reservation.Email, &reservation.Phone,
		&reservation.StartDate, &reservation.EndDate, &reservation.RoomID, &reservation.CreatedAt, &reservation.UpdatedAt,
		&reservation.Processed, &reservation.Room.ID, &reservation.Room.RoomName)
...
func (m *postgresDBRepo) DeleteteReservation(id int) error {
...
func (m *postgresDBRepo) InsertBlockForRomm(id int, startDate time.Time) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	nextDateString := startDate.AddDate(0, 0, 1).Format("2006-01-02")

	_, err := m.DB.ExecContext(ctx, query, startDateString, nextDateString, id, 2, time.Now(), time.Now())
